...
public class ExtendedBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public abstract static class BottomSheetCallback {
         ...
         *                    {@link #STATE_SETTLING}, {@link #STATE_EXPANDED}, {@link #STATE_HALF},
         *                    {@link #STATE_COLLAPSED}, or {@link #STATE_HIDDEN}.
         */
        public abstract void onStateChanged(@NonNull View bottomSheet, @ExtendedBottomSheetBehavior.State int newState);
     ...
     * The bottom sheet is expanded.
     */
    public static final int STATE_HALF = 6;
    ...
    private ExtendedBottomSheetBehavior.BottomSheetCallback mCallback;
    ...
    public ExtendedBottomSheetBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ExtendedBottomSheetBehavior);
        TypedValue value = a.peekValue(R.styleable.ExtendedBottomSheetBehavior_peekHeight);
        if (value != null && value.data == PEEK_HEIGHT_AUTO) {
            setPeekHeight(value.data);
        } else {
            setPeekHeight(a.getDimensionPixelSize(R.styleable.ExtendedBottomSheetBehavior_peekHeight, PEEK_HEIGHT_AUTO));
        }
    ...
    public void onStopNestedScroll(@NonNull CoordinatorLayout coordinatorLayout, @NonNull V child, @NonNull View target) {
        ...
        if (mViewDragHelper.smoothSlideViewTo(child, child.getLeft(), top)) {
            ...
            ViewCompat.postOnAnimation(child, new ExtendedBottomSheetBehavior.SettleRunnable(child, targetState));
        } else {
     ...
     */
    public void setBottomSheetCallback(ExtendedBottomSheetBehavior.BottomSheetCallback callback) {
        mCallback = callback;
    }
     ...
     * animation.
     *
     * @param state One of {@link #STATE_COLLAPSED}, {@link #STATE_EXPANDED}, or {@link #STATE_HIDDEN}, or(@link #STATE_HALF).
     */
    public final void setState(final @ExtendedBottomSheetBehavior.State int state) {
        if (state == mState) {
            return;
        }
        if (mViewRef == null) {
            // The view is not laid out yet; modify mState and let onLayoutChild handle it later
            if (state == STATE_COLLAPSED || state == STATE_EXPANDED || state == STATE_HALF || (mHideable && state == STATE_HIDDEN)) {
                mState = state;
            }
            return;
        }
     ...
     *
     * @return One of {@link #STATE_EXPANDED}, {@link #STATE_HALF}, {@link #STATE_COLLAPSED}, {@link #STATE_DRAGGING},
    ...
    private void setStateInternal(@ExtendedBottomSheetBehavior.State int state) {
        if (mState == state) {
            return;
        }
        mState = state;
    ...
    private void startSettlingAnimation(View child, int state) {
        ...
        setStateInternal(STATE_SETTLING);
        if (mViewDragHelper.smoothSlideViewTo(child, child.getLeft(), top)) {
            ViewCompat.postOnAnimation(child, new ExtendedBottomSheetBehavior.SettleRunnable(child, state));
        }
    }
    ...
    private final ViewDragHelper.Callback mDragCallback = new ViewDragHelper.Callback() {
        ...
        public void onViewReleased(@NonNull View releasedChild, float xvel, float yvel) {
            ...
            if (mViewDragHelper.settleCapturedViewAt(releasedChild.getLeft(), top)) {
                setStateInternal(STATE_SETTLING);
                ViewCompat.postOnAnimation(releasedChild, new ExtendedBottomSheetBehavior.SettleRunnable(releasedChild, targetState));
            } else {
                setStateInternal(targetState);
            }
        }
    ...
    private class SettleRunnable implements Runnable {
        ...
        SettleRunnable(View view, @ExtendedBottomSheetBehavior.State int targetState) {
            mView = view;
    ...
    private static class SavedState extends AbsSavedState {
        ...
        private SavedState(Parcelable superState, @ExtendedBottomSheetBehavior.State int state) {
            super(superState);
            this.state = state;
        }
        ...
        public static final Creator<ExtendedBottomSheetBehavior.SavedState> CREATOR = ParcelableCompat.newCreator(
                new ParcelableCompatCreatorCallbacks<ExtendedBottomSheetBehavior.SavedState>() {
                    @Override
                    public ExtendedBottomSheetBehavior.SavedState createFromParcel(Parcel in, ClassLoader loader) {
                        return new ExtendedBottomSheetBehavior.SavedState(in, loader);
                    }

                    @Override
                    public ExtendedBottomSheetBehavior.SavedState[] newArray(int size) {
                        return new ExtendedBottomSheetBehavior.SavedState[size];
                    }
                });
    }
